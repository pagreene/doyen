...
class TestPubMedProcessor(unittest.TestCase):
    ...
    def test_download_and_convert_to_xml_files(self):
        ...
        converted_files = self.ftpDownloader.download_and_convert_to_xml_files(
            files_to_download_and_convert
        )
    ...
    def test_download_file_and_convert_to_xml(self):
        ...
        # Assert that requested file is downloaded to the download directory
        self.assertTrue(
            os.path.exists(os.path.join(download_dir, xml_file_to_download))
        )
    ...
    def test_download_latest_files_by_count(self):
        ...
        downloaded_files = self.ftpDownloader.download_latest_files_by_count(
    ...
    def test_index_pubmed_files(self):
        ...
        files_to_index = [
            os.path.join(local_dir, "pubmed23n1114.xml.gz"),
            os.path.join(local_dir, "pubmed23n1115.xml.gz"),
        ]
        # Following code is to process and index all the gz files in the pubmed_folder
        files_indexed = index_pubmed_files(files_to_index)
