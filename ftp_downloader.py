...
class FtpDownloader:
    ...
    def __init__(
        self, host, ftp_dir, download_dir, user_id=None, pwd=None, reg_ex=".*"
    ):
        self.host = host
    ...
    def download_and_convert_to_xml_files(
        self, numberOfFiles=10, deleteGzFilesAfterConverting=False
    ):
            ...
            if filename.endswith(".gz"):
                with gzip.open(os.path.join(download_dir, filename), "rb") as f_in:
                    with open(os.path.join(download_dir, filename[:-3]), "wb") as f_out:
                        ...
                        print(
                            f"converted {os.path.join(download_dir, filename)} to {os.path.join(download_dir, filename[:-3])}"
                        )
